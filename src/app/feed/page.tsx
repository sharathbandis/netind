...
import { LogOut, Home, Users, Bell, Search, Send, Settings, Heart, BadgeCheck, MessageCircle, Image as ImageIcon, X, Repeat, Loader2, Bookmark, TrendingUp } from "lucide-react";
  ...
  return (
    <div className={`flex gap-3 ${indentClass}`}>
      <div className="w-8 h-8 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center text-slate-600 dark:text-slate-400 font-bold overflow-hidden shrink-0 text-xs mt-1">
        {cProfile?.avatar_url ? (
          <img src={cProfile.avatar_url} alt="Avatar" className="w-full h-full object-cover" />
        ) : (
          cProfile?.full_name ? cProfile.full_name.charAt(0).toUpperCase() : "U"
        )}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 mt-1 ml-2">
          <button 
            onClick={() => onReplyClick(comment.id, cProfile?.username || cProfile?.full_name)} 
            className="text-xs font-bold text-slate-500 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors"
          >
            Reply
          </button>
...
export default function Feed() {
    ...
    if (data) {
      const sortedData = data.map(post => {
        ...
        return { 
          ...
          rawCommentCount: flatComments.length 
        };
      });
      setPosts(sortedData);
    }
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const combinedFiles = [...mediaFiles, ...newFiles].slice(0, 4); // Cap at 4 files
    ...
    try {
      if (mediaFiles.length > 0) {
        for (const file of mediaFiles) {
          ...
          const { error: uploadError } = await supabase.storage.from('post_media').upload(fileName, file);
      ...
      const { error } = await supabase.from('posts').insert([{ 
        ...
        media_attachments: uploadedMedia,
        image_url: uploadedMedia.length > 0 && uploadedMedia[0].type === 'image' ? uploadedMedia[0].url : null
      }]);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 font-sans transition-colors duration-300">
      ...
      <nav className="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-4 md:px-8 py-3 flex items-center justify-between transition-colors duration-300">
          ...
          {searchQuery.trim() !== "" && (
            <div className="absolute top-full mt-2 left-0 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-lg overflow-hidden z-50">
              {searchResults.length === 0 && !isSearching ? (
                <div className="p-4 text-center text-sm text-slate-500">No rebels found.</div>
              ) : (
                    ...
                    <Link key={result.id} href={`/profile/${result.id}`} onClick={() => setSearchQuery("")} className="flex items-center gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors border-b border-slate-100 dark:border-slate-800/50 last:border-0">
                      ...
                      <div className="truncate">
                        <div className="flex items-center gap-1">
                          <span className="text-sm font-bold text-slate-900 dark:text-slate-200 truncate">{result.full_name || "Unknown"}</span>
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          <Link href="/bookmarks" className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
                        ...
                        <div key={notif.id} className={`flex items-start gap-3 p-3 border-b border-slate-50 dark:border-slate-800/30 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${!notif.is_read ? 'bg-indigo-50/50 dark:bg-indigo-900/10' : ''}`}>
                          ...
                          <div>
                            <p className="text-sm text-slate-800 dark:text-slate-200">
                              <Link href={`/profile/${notif.actor_id}`} className="font-bold hover:underline">{actor.full_name || "Someone"}</Link> {actionText}
                            </p>
            ...
            <Link href={`/profile/${user.id}`} className="hover:opacity-80 transition-opacity">
              <div className="w-7 h-7 md:w-8 md:h-8 rounded-full overflow-hidden bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 flex items-center justify-center">
                {currentProfile?.avatar_url ? (
                  ...
                  <span className="text-xs font-bold text-slate-600 dark:text-slate-400">{currentProfile?.full_name?.charAt(0).toUpperCase() || user?.email?.charAt(0).toUpperCase() || "U"}</span>
                )}
              </div>
            </Link>
          )}
        ...
        <div className="flex-1 min-w-0 w-full max-w-2xl">
          ...
          <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-4 transition-colors duration-300 shadow-sm dark:shadow-none mb-6">
            <form onSubmit={handleCreatePost}>
              ...
              {/* Multi-Media Previews Container */}
              {mediaPreviews.length > 0 && (
                <div className={`mt-3 grid gap-2 ${mediaPreviews.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                  {mediaPreviews.map((preview, index) => (
                    <div key={index} className="relative rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 aspect-video md:aspect-auto">
                      {preview.type === 'video' ? (
                        <video src={preview.url} className="w-full h-full object-cover max-h-48" muted />
                      ) : (
                        <img src={preview.url} alt={`Preview ${index}`} className="w-full h-full object-cover max-h-48" />
                      )}
                      <button type="button" onClick={() => removeMedia(index)} className="absolute top-1 right-1 bg-slate-900/80 text-white rounded-full p-1 hover:scale-110 transition-transform backdrop-blur-sm">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
              ...
              <div className="flex justify-between items-center mt-3 pt-3 border-t border-slate-100 dark:border-slate-800/50">
                <label className="cursor-pointer p-2 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-full transition-colors flex items-center gap-2 text-sm font-medium">
                  ...
                  <span className="hidden sm:inline">Media (Up to 4)</span>
                  <input type="file" multiple accept="image/*,video/*" onChange={handleFileSelect} className="hidden" disabled={isPublishing || mediaFiles.length >= 4} />
                </label>
                <button type="submit" disabled={isPublishing || (!newPost.trim() && mediaFiles.length === 0)} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-5 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm">
                ...
                posts.map((post) => {
                  ...
                  if (post.original_post_id) {
                     ...
                     if (localOriginal) { 
                       ...
                       displayPost = { ...post, content: "[This original post was deleted]", image_url: null, media_attachments: [] }; 
                       displayProfile = null; 
                     }
                  }
                  if (Array.isArray(displayProfile)) displayProfile = displayProfile[0];

                  // Consolidate legacy image_url and new media_attachments
                  let mediaArray = displayPost.media_attachments || [];
                  if (mediaArray.length === 0 && displayPost.image_url) {
                    mediaArray = [{ url: displayPost.image_url, type: 'image' }];
                  }
                    ...
                    <div key={post.id} className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-5 transition-colors duration-300 shadow-sm dark:shadow-none">
                      ...
                      <div className={isRepost ? "border border-slate-100 dark:border-slate-800/80 rounded-xl p-4 bg-slate-50/50 dark:bg-slate-900/30" : ""}>
                          ...
                          <Link href={`/profile/${displayPost.user_id}`} className="flex items-start gap-3 hover:opacity-80 transition-opacity">
                            <div className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center text-slate-600 dark:text-slate-400 font-bold transition-colors duration-300 mt-1 overflow-hidden shrink-0">
                              {displayProfile?.avatar_url ? (
                                ...
                                displayPost.author_name ? displayPost.author_name.charAt(0).toUpperCase() : "U"
                              )}
                            </div>
                            <div>
                        ...
                        {displayPost.content && (
                          <p className="text-slate-700 dark:text-slate-300 text-sm whitespace-pre-wrap leading-relaxed mb-4">
                            {renderContentWithTags(displayPost.content)}
                          </p>
                        )}
                        
                        {/* Multi-Media Grid Renderer */}
                        {mediaArray.length > 0 && (
                      ...
                      <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800/50 pt-3 mt-3">
                        <div className="flex items-center gap-6">
                          <button 
                            onClick={() => handleLike(displayPost.user_id, post.id, userHasLiked)} 
                          ...
                          {!isRepost && (
                            <button 
                              onClick={() => handleRepost(displayPost.user_id, post.id)} 
                              className="flex items-center gap-1.5 text-sm transition-colors text-slate-500 dark:text-slate-400 hover:text-green-600 dark:hover:text-green-400"
                            >
                        ...
                        <button 
                          onClick={() => handleBookmark(post.id, userHasBookmarked)} 
                          className={`p-1.5 rounded-full transition-colors ${userHasBookmarked ? 'text-amber-500' : 'text-slate-500 dark:text-slate-400 hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/30'}`}
                        >
              ...
              <div className="space-y-4">
                {trendingTags.map((tagObj, index) => (
                  <Link key={index} href={`/tag/${tagObj.tag.slice(1)}`} className="block group">
                    ...
                    <p className="font-bold text-slate-900 dark:text-slate-200 group-hover:text-indigo-500 transition-colors">
                      {tagObj.tag}
                    </p>
