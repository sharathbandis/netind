  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        router.push("/feed");
        // Later we will redirect to the feed page here
      } else {
        ...
        if (error) throw error;
        setMessage({ text: "Welcome to the netind. Check your email to verify your account!", type: "success" });
      }
    } catch (error: any) {
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-950 text-slate-50 font-sans selection:bg-indigo-500/30">
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-center p-8 md:p-16 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 text-sm text-slate-500 bg-slate-900/50 w-fit px-4 py-2 rounded-full border border-slate-800">
          ...
          <span>Take back control of your professional identity.</span>
        </div>
      </div>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="text-center">
            ...
            <p className="text-slate-400">
              {isLogin ? "Enter your details to access your network." : "Create your account to claim your handle."}
            </p>
          </div>
          ...
          <form className="space-y-4 mt-8" onSubmit={handleAuth}>
            ...
            {!isLogin && (
              <div className="relative">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-3 py-3 border border-slate-800 rounded-xl bg-slate-900/50 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
            ...
            {message && (
              <div className={`p-3 rounded-lg text-sm ${message.type === 'error' ? 'bg-red-900/50 text-red-200 border border-red-800' : 'bg-green-900/50 text-green-200 border border-green-800'}`}>
                {message.text}
              </div>
            )}
