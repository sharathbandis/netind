  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          // This tells Google to send the user straight to the feed after logging in
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (error) throw error;
        setMessage({ text: "Account created! Redirecting to feed...", type: "success" });
        router.push("/feed"); // Since we turned off email verification, we slide them right in
      }
    } catch (error: any) {
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center p-8 border-t md:border-t-0 md:border-l border-slate-800 bg-slate-950/50">
        <div className="w-full max-w-md space-y-6">
          ...
          <button
            ...
            disabled={loading}
            type="button"
            className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl font-medium text-slate-900 bg-white hover:bg-slate-100 transition-all disabled:opacity-50 mt-8"
          >
